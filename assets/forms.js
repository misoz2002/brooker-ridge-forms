document.addEventListener('DOMContentLoaded',()=>document.querySelectorAll('.brah-form').forEach(form=>{
  ...
  form.addEventListener('change',sync);form.addEventListener('submit',()=>{sync();const button=form.querySelector('button[type="submit"]');if(button){button.disabled=true;button.textContent='Sending…'}});sync();
}));
