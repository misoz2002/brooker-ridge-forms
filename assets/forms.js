document.addEventListener('DOMContentLoaded',()=>document.querySelectorAll('.brah-form').forEach(form=>{
  ...
  const valueOf=name=>{const controls=[...form.querySelectorAll('[name]')].filter(x=>x.name===name||x.name===name+'[]'),choices=controls.filter(x=>x.type==='checkbox'||x.type==='radio');if(choices.length)return choices.filter(x=>x.checked).map(x=>x.value);return controls[0]?.value??''};
